...
import { ArrowLeft, CircleAlert, CircleCheck, CircleX, Eye, EyeClosed, Key, MoveLeft } from "lucide-react";
...
export default function ResetPassword() {
  ...
  useEffect(() => {
    if (hasCompletedValidation && !isValidating && !success) {
      const nextElement = (error && !success) 
        ? invalidLinkRef.current 
        : formContainerRef.current;
        
      if (!nextElement) {
        console.log('target container NOT find!\nThis is an error, REPORT IT !');
        return;
      }
  ...
  const handlePageExit = () => {
    return new Promise<void>((resolve) => {
      ...
      if (formContainerRef.current && getComputedStyle(formContainerRef.current).display !== 'none') {
        element = formContainerRef.current;
      } else if (invalidLinkRef.current && getComputedStyle(invalidLinkRef.current).display !== 'none') {
        element = invalidLinkRef.current;
      } else if (loadingRef.current && getComputedStyle(loadingRef.current).display !== 'none') {
    ...
    <ProtectedRoute requireAuth={false}>
      <PageTransition>
        ...
        <div className="bg-gradient-to-tr from-indigo-900 from-25% to-sky-800 relative overflow-hidden flex flex-1 flex-col items-center justify-center bg-cover bg-center bg-no-repeat border-t-0 rounded-b-5xl w-full shadow-2xl border-b-2 border-b-gray-500 tracking-wider">
          {isValidating && (
            <div ref={loadingRef} className="flex flex-col items-center justify-center relative border-4 border-white/20 w-[90%] sm:w-2/3 lg:w-1/3 xl:w-1/4 min-h-28 rounded-xl px-8 py-6 bg-white/5 backdrop-blur-xl shadow-2xl font-medium tracking-wide">
              <Spinner size="lg" color="default" label="正在驗證重設連結" classNames={{ label:"text-white font-bold text-xl" }} />
            </div>
          )}
          ...
          >
            ...
            <div className="text-2xl font-bold">無效的重設連結</div>
            <div className="text-gray-300 text-base">{error || "此重設連結無效或已過期"}</div>
            <Link
              href="/login"
              prefetch={false}
              className="w-[180px]"
          ...
          >
            ...
            {userEmail && (
              <div className="text-gray-300 text-sm pb-6 text-center">
                為以下帳號設定一組新密碼
                <div className="font-semibold text-sky-300">{userEmail}</div> 
              </div>
            )}
          ...
          >
            <div className="flex-shrink-0">
              { success ? <CircleCheck size={16}/> : <CircleAlert size={16}/> }
            </div>
          ...
          <div className={`w-full max-w-md flex flex-col items-center ${!passwordError && !confirmPasswordError ? 'space-y-6' : 'space-y-4'}`} style={{transition: 'gap 0.3s ease-out, margin 0.3s ease-out, padding 0.3s ease-out'}}>
            <div className="relative w-full custom-input-trans-animate">
              <CustomInput
                ...
                errorMessage={passwordError ? `*${passwordError}` : undefined}
              />
              <Button
                isIconOnly
                variant="light"
            ...
            <div className="relative w-full custom-input-trans-animate">
              <CustomInput
                ...
                errorMessage={confirmPasswordError ? `*${confirmPasswordError}` : undefined}
              />
              <Button
                isIconOnly
                variant="light"
            ...
            <div className="flex w-full justify-center">
              <CustomButton 
                ...
                size="lg"
                radius="full"
                startContent={!isLoading ? <Key size={20}/> : undefined}
