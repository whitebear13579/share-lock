...
import { CircleAlert, CircleCheck, CircleQuestionMark, Eye, EyeClosed, LogIn } from "lucide-react";
  ...
  return (
    <ProtectedRoute requireAuth={false}>
      ...
      <div className="bg-gradient-to-tr from-indigo-900 from-25% to-sky-800 relative overflow-hidden flex flex-1 flex-col items-center justify-center bg-cover bg-center bg-no-repeat border-t-0 rounded-b-5xl w-full shadow-2xl border-b-2 border-b-gray-500 tracking-wider">
        <div ref={formContainerRef} className="flex flex-col items-center justify-center relative border-4 border-white/20 w-[90%] sm:w-2/3 lg:w-1/3 xl:w-1/4 min-h-28 rounded-xl px-8 py-6 bg-white/5 backdrop-blur-xl shadow-2xl font-medium tracking-wide" style={{transition: 'height 0.3s ease-out, min-height 0.3s ease-out'}}>
          ...
          >
            <div className="flex-shrink-0">
              { resetEmailSent ? <CircleCheck size={16}/> : <CircleAlert size={16}/> }
            </div>
            <span className="leading-relaxed break-words">
              {error}
            </span>
          </div>
          <div className={`w-full max-w-md flex flex-col items-center ${!emailError && !passwordError ? 'space-y-8' : 'space-y-6'}`} style={{transition: 'gap 0.3s ease-out, margin 0.3s ease-out, padding 0.3s ease-out'}}>
            ...
            <div className="flex w-full justify-between items-center gap-4">
              ...
              <CustomButton 
                ...
                size="lg"
                radius="full"
                startContent={!isLoading ? <LogIn size={20}/> : undefined}
